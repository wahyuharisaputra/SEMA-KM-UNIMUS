    ...
    <div class="card-body">
        <form action="{{ route('admin.galeri.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-3">
                ...
                <input class="form-control @error('file_foto') is-invalid @enderror" type="file" id="file_foto" name="file_foto" required accept="image/*">
                <div class="form-text">Maksimal ukuran file: 5MB. Format: JPG, PNG, JPEG.</div>
                @error('file_foto')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            
            <button type="submit" class="btn btn-primary"><i class="bi bi-save"></i> Unggah Foto</button>
        </form>
    </div>
</div>
@endsection
